...
function handleClick(clickedButtonEvent) {
    ...
    let text_on_button = card.button.innerText
    
    if (text_on_button == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (text_on_button == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let random_text = 'Lorem ipsum dolor sit, amet consectetur adipisicing elit.\
    Repudiandae exercitationem numquam nostrum dignissimos cupiditate?\
    Officia pariatur libero dolore numquam doloribus distinctio corporis ipsam eligendi delectus,\
    neque, voluptas maxime sapiente illo?'
